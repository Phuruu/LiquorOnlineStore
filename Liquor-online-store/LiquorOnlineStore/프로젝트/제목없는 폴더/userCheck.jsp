...
function joinGo(frm) {
	...
	var regExp5 = /^.*(?=^.{8,15}$)(?=.*\d)(?=.*[a-zA-Z])(?=.*[!@#$%^&+=]).*$/; //비밀번호 (8자 이상, 소문자/대문자/특수문자/숫자 포함 
     ...
     if (frm.userId.value==""){
    	 ...
    	 alert("생년월일을 입력해주세요.");
    	 return false;
     }else if(frm.idDuplication.value != 1) {
    	 alert(frm.idDuplication.value);
    	alert("아이디 중복체크를 해주세요.");
     	return false;
     } /* else if(frm.nickDuplication.value!="닉네임 중복확인") {
				...
				<tr>
					...
					<td>
						<div>
							...
							<!-- 아이디 중복 체크 여부 -->
							<input type = "hidden" name="idDuplication" value="${userIdCheck }" />
						</div>
					</td>
				</tr>
				<tr>
				    ...
				    <th height="50px">생년월일</th>
				  	<td>
               		<div class="birth">
                        ...
                        <div id="bir_dd" >
                            <span class="box">
                                <input type="text" id="dd" name="ubirth3" class="int" maxlength="2" style="text-align: right; height: 24px;">일
                            </span>
                        </div>
